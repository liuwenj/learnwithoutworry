    ...
    <div id="insuranceInformation">
        ...
        <div v-for="(item,index) in List" :key=index>
            <div class="insuranceInformation_main">
                <span>{{item.name}}</span>
                <input type="text" :placeholder='item.name' v-model="item.key" id="inputData" :name="inputVal" @blur="getFocus">

            </div>
        </div>

        <!-- <div v-if="tbNameStatus ==1 " class="msg_detail" style="width:100%;height:0.89rem;background:rgba(255,241,240,1);">
            <div style="display:inline-block;width：0.6rem;"></div>
          {{tbNameMsg}}
        </div> -->
        <!-- <div class="insuranceInformation_main" >
            <span>投保人名称</span>
            <input type="text" placeholder="投保人名称" v-model="CreditCode" @blur="tbNameMethod">

        </div> -->
        <!-- <div class="insuranceInformation_main" >
            <span>社会统一信用代码</span>
            <input type="text" placeholder="社会统一信用代码" v-model="CreditCode" @blur="CreditCodeMethod">

        </div>
         <div v-if="CreditCodeStatus ==1 " class="msg_detail" style="width:100%;height:0.89rem;background:rgba(255,241,240,1);">
            <div style="display:inline-block;width：0.6rem;"></div>
          {{CreditCodeMsg}}
        </div>
        <div class="insuranceInformation_main">
            <span>联系人姓名</span>
            <input type="text" placeholder="联系人姓名"  v-model="ContactName" @blur="ContactNameMethod">

        </div>
         <div v-if="ContactNameStatus ==1 " class="msg_detail" style="width:100%;height:0.89rem;background:rgba(255,241,240,1);">
            <div style="display:inline-block;width：0.6rem;"></div>
          {{ContactNameMsg}}
        </div>
        <div class="insuranceInformation_main">
            <span>联系人电话</span>
            <input type="text" placeholder="联系人电话" v-model="phone" @blur="phoneMethod">

        </div>
        <div v-if="phoneStatus ==1 " class="msg_detail" style="width:100%;height:0.89rem;background:rgba(255,241,240,1);">
            <div style="display:inline-block;width：0.6rem;"></div>
          {{phoneMsg}}
        </div>
        <div class="insuranceInformation_main">
            <span>联系人邮箱</span>
            <input type="text" placeholder="联系人邮箱" v-model="mail" @blur="mailMethod">
        </div>
        <div v-if="mailStatus ==1 " class="msg_detail" style="width:100%;height:0.89rem;background:rgba(255,241,240,1);">
            <div style="display:inline-block;width：0.6rem;"></div>
            {{mailMsg}}
        ...
        <van-popup v-model="show" position="bottom" :style="{ height: '50%' }">
            <van-area :area-list="areaList" :columns-num="2" ref="myArea" title="出发城市" @change="onChange" @confirm="onConfirm" @cancel="onCancel" />
        </van-popup>
        <div class="insuranceInformation_main" style=" height: 1rem">
            ...
            <textarea rows="3" cols="10" style="width:4.3rem;font-size:0.27rem;border:none;text-align:left;padding-top: 0.3rem;" placeholder="请选择研学城市" v-model="learnCity" @click="showPopups" readonly="readonly">
        ...
        <van-popup v-model="showss" position="bottom" :style="{ height: '50%' }">
            <van-area :area-list="areaList" :columns-num="2" ref="myAreas" title="研学城市(可多选)" @change="onChanges" @confirm="onConfirms" @cancel="onCancels" />
        </van-popup>

        <p style="#2AB5CA;font-size:0.28rem;color:#2AB5CA;padding-left:0.27rem;margin-top:0.15rem">开票信息（必填）</p>
...
export default {
    ...
    created() {
        ...
        let that = this;
        this.$http
            .get("/getProductInfo?productUuid=2c91edc26f35bf25016f84b4339031c4")
            .then(function(res) {
                // console.log(res);
    ...
    methods: {
        ...
        gos() {
            ...
            let dataList = {
                ...
                researchCity: this.learnCity,
                invoices: this.radio,
                businessType: this.radio_a,
                invoiceForm: this.radio_b,
                invoiceType: this.radio_c,
                orderTime: this.orderTime
            };

            let data = { entity: JSON.stringify(dataList) };

            console.log(data);

            if (this.address == "") {
                this.$notify({ type: "danger", message: "请填写出发城市" });
            } else if (this.learnCity == "") {
        ...
        onChange(picker, index, value) {
            ...
            for (var i = 0; i < val.length; i++) {
                areaName = areaName + (i == 0 ? "" : "/") + val[i + 1].name;
            }
        ...
        onConfirms(val) {
            ...
            let num =
                this.Citys.substr(0, this.Citys.length - 1).split(",").length -
                1;
            if (num > 9) {
                this.$notify({
                    type: "danger",
